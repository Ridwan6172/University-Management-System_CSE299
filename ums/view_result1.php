...
<div class="all_student fix">
	...
	<div>
	<p style="float:left;margin:0 0 5px 0;width:100%;text-align:center;"><a href="view_cgpa.php?vr=<?php echo $stid; ?>&vn=<?php echo $name; ?>"><button class="editbtn"><font color=#000000>	View CGPA & completed course</button></a></p>
	</div>
	...
	<?php
			...
			$gp = 0;
				...
				if($get_result){
			?>
				<p><font color=#000000>	<?php echo "<p style='text-align:center;background:#ddd;color:#01C3AA;padding:5px;width:84%;margin:0 auto'>".$semester." Semester Result"?></p>
    ...
    tr:nth-child(even) {
		...
		?>
			...
			<?php } ?>
			<tr>
				<td colspan="2"><b>CGPA : </td>
				<td colspan="2"><font color=#000000>
				<?php
				$sg = $gp/$ch;
				echo "<span style='color:white;padding:3px 6px;font-size:20px'>" . round($sg,2) . "</span>"; ?>
				</td>
				<td><font color=#000000>
					<?php
						if($sg>=3.5){
		...
		?>
			<p style="float:left; text-align:right;margin:20px 0;width:49%"><a href="st_result_update.php?ar=<?php echo $stid?>&seme=<?php echo $semester?>&vn=<?php echo $name?>"><button class="editbtn"><font color=#000000>	Edit Result</button></a></p>
		<?php 
				}
		?>
		
			<p style="float:right;text-align:left;margin:20px 0;width:49%"><a href="st_result.php"><button class="editbtn"><font color=#000000>	Back to list</button></a></p>
